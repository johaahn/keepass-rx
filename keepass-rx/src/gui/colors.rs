...
pub enum RxColorType {
    #[default]
    Light,
    Dark,
}

impl ToString for RxColorType {
    fn to_string(&self) -> String {
        match self {
            RxColorType::Dark => "Dark",
            RxColorType::Light => "Light",
        }
        .into()
    }
}

fn type_to_string(color_type: &RxColorType) -> QString {
    match color_type {
        RxColorType::Dark => "Dark",
        RxColorType::Light => "Light",
    }
...
fn string_to_type(qstr: &QString) -> RxColorType {
    match qstr.to_string().as_str() {
        "Dark" => RxColorType::Dark,
        "Light" => RxColorType::Light,
        _ => panic!("Invalid color type: {}", qstr.to_string()),
    }
}

impl QMetaType for RxColorType {
...
pub fn text_color_for_background(hex: &str) -> Result<RxColorType, String> {
    ...
    if normalized_l > 0.5 {
        Ok(RxColorType::Dark) // background is light → use dark text
    } else {
        Ok(RxColorType::Light) // background is dark → use text text
    }
}
